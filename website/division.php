...
if (!isset($_GET['date'],$_GET['number'])) {
    if (isset($_SERVER['HTTP_REFERER'])) {
        if (strpos($_SERVER['HTTP_REFERER'],'publicwhip.org.uk')!==false) {
            error_log('Request to division.php made without parameters: Referer: '.$_SERVER['HTTP_REFERER'].' URL: '.$_SERVER['REQUEST_URI']);
        }
    }
    header('Location: /divisions.php?error=noDivisionSpecified');
    exit;
}
function no_division_found($plural)
{
    ...
    print 'Looked for house '.htmlspecialchars($_GET['house']).' on '.htmlspecialchars($_GET['date']).' number '.htmlspecialchars($_GET['number']);
    pw_footer();
    exit;
}
...
$motion_data = get_wiki_current_value("motion",
    array($divattr["division_date"], $divattr["division_number"], $divattr['house']));
...
if ($singlemotionpage) {
    ...
    $dismodes["similardivisionsparl"] = array(
        "dtype" => "similardivisionsparl",
...
if ($dismode["summarytext"]) {
    ...
    */
    ...
    if ($votertype === "mp") {
        ...
        print '<p class="mpondivision">';
        //print '<a href="/mp/'.$house.'/for/'.urlencode($voter['constituency']).'/name/'.urlencode($voter['name']).'">';
        print "<a href=\"/mp.php?" . $voter['mpanchor'] . "\">";
        print $voter['name'] . ($house == "lords" ? "</a>" : " MP, " . $voter['constituency'] . "</a>");
        if ($actiontext[$vote]) {
                        ...
                        if ($vote == 'tellaye') {
                            ...
                            if ($vote == 'tellno') {
                                ...
                                if ($vote == 'both') {
                                    ...
                                    if (($vote == 'abstention') and ($house == 'scotland')) {
                                        print " abstained";
                                    } else {
...
if ($dismode["motiontext"]) {
    if ($singlemotionpage) {
        ...
        if ($divattr["division_date"] <= '2005-03-17') {
            $millbankurl = $divattr["house"] . "/" . substr($divattr["division_date"], 0,
                    4) . "/" . substr($divattr["division_date"], 5, 2) . "/" . substr($divattr["division_date"], 8,
                    2) . "/division_" . $divattr["division_number"];
        ...
        $motion_data_a = get_wiki_current_value("motion",
            array($divattr["division_date"], $divattr["division_number"], $divattr['house']));
        ...
        $motion_data_b = get_wiki_current_value("motion",
            array($divattr2["division_date"], $divattr2["division_number"], $divattr2['house']));

...
if ($rdisplay_house !== "all") {
    ...
    if ($rdisplay !== 'all') {
        $placeholders[':fromdate'] = $parliaments[$rdisplay]["from"] ?: '1997-01-01';
        $placeholders[':todate'] = $parliaments[$rdisplay]["to"] ?: '9999-12-31';
    };
    $pwpdo->query($query, $placeholders);
    while ($row = $pwpdo->fetch_row()) {
        $party = $row["party"];
        if ($party != "CWM" && $party != "DCWM" && substr($party, 0,
                3) != "Ind" && $party != "Other" && $party != "None" && $party != "SPK") {
...
foreach ($rdismodes2 as $lrdisplay => $lrdismode) {
    ...
    array_push($second_links3, array(
        'href' => $dlink,
...
foreach ($rdismodes_house as $lrdisplay_house => $lrdismode) {
    ...
    array_push($second_links2, array(
        'href' => $dlink,
...
}  # this extra bit of information only shows up for advanced users who are looking at all parliaments

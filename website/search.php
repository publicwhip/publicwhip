    ...
    if ($number_of_matches === 1) {
        ...
        foreach (array("WMC", "SPC", "SPE") as $k) {
            ...
            foreach ($rows as $row) {
                ...
                $constituency_url = "/mp.php?mpc=" . urlencode(str_replace(" ", "_",
                        $row['constituency'])) . "&" . "house=" . urlencode($row['house']);
                print '<tr>'.PHP_EOL;
...
{
?>
<p>Nothing found matching <?php if ($postcode) {
        print "postcode ";
    } ?> '<?php echo $prettyquery; ?>'.
    <?php
    }
...
<p class="search"><i>Example: "OX1 3DR", "<?php echo $random_mp ?>"<?php echo $random_constituency ?> or
        "<?php echo $random_topic ?>"</i>

...
$dismodes["comparison"] = array(
    "description" => "Compare with MPs",
    "comparisons" => "slab",
    "divisionlist" => "selected", # those which are seen out of the total
...
$dismodes["editdefinition"] = array(
    "description" => "Edit",
    "editdefinition" => "yes",
    "tooltip" => "Change title and definition of policy"
);

$dismodes["discussion"] = array(
...
if ($dismode["editdefinition"]) {
    if (!$adminAllowed) {
        pw_handle_404('Discussion editing currently disabled','Sorry - discussion editing is currently restricted');
        die();
    }
    ...
    if (($private == 1) && user_getid() != $user_id) {
        ...
        if ($name == "" or $description == "") {
            ...
            dream_post_forum_action($db, $dreamid,
                "Changed name and/or definition of policy.\n\n[b]Name:[/b] " . $name_diff . "\n[b]Definition:[/b] " . $description_diff);
...
if ($_GET["savevotes"] && $bAggregateEditable && $bAggregate) {
    if (!$adminAllowed) {
        pw_handle_404('Saving votes currently disabled','Saving votes is currently restricted');
        exit();
    }
...
if ($dismode["discussion"]) {
    if (!$adminAllowed) {
        pw_handle_404('Discussions currently disabled','Discussions are currently restricted');
        exit();
    }
    ...
    if ($discuss_url) {
        print '<iframe src="' . htmlspecialchars($discuss_url,ENT_QUOTES) . '" width="100%" height="10000" scrolling="no">';
        print '<a href="' . htmlspecialchars($discuss_url,ENT_QUOTES) . '">Click here for discussion</a>';
        print '</iframe>';
    }
}
...
if ($dismode['editdefinition']) {
    if (!$adminAllowed) {
        pw_handle_404('Edit definition currently disabled','Edit definitions are currently restricted');
        exit();
    }
    if (!user_getid()) {
    ...
    if (!$ok) {
        ...
        <p><span class="ptitle">Title:</span> <INPUT TYPE="TEXT" NAME="name" VALUE="<?php echo html_scrub($name) ?>"
                                                     SIZE="40" MAXLENGTH="50">
        <P>
        <span class="ptitle">Text:</span> Someone who believes that<BR>
        <textarea class="policytext" name="description" rows="2"
                  cols="80"><?php echo htmlspecialchars($description,ENT_QUOTES) ?></textarea>
        <br>
        ...
        <?php if (!$legacy_dream) { ?>
            <p>
                <INPUT TYPE="checkbox" NAME="provisional" value="provisional"
                       id="provisional" <?php echo $provisional ? 'checked' : '' ?>>
...
if ($dismode["aggregate"] == "fulltable") {
    if (!$adminAllowed) {
        pw_handle_404('Full table aggregate stats currently disabled','Full table aggregate stats currently restricted');
        die();
    }
    // changed vote
...
if ($dismode["aggregate"] == "shown") {
    ...
    if ($dismode["divisionlist"] == "bothdiff" && $dismode["dtype"]) {
...
if ($dismode["policybox"]) {
    ...
    print htmlspecialchars(dream_box($dreamid, $policyname),ENT_QUOTES);
    print '</pre>';
}

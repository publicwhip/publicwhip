...
function guess_whip_for_all()
{
    ...
    $divisions = $pwpdo->fetch_all_rows('SELECT division_id AS id, house FROM pw_division', array());
    foreach ($divisions as $division) {
        ...
        $qwhere = " WHERE pw_division.division_id=:divid";
        $qgroup = " GROUP BY pw_mp.party";
        $query = $qselect.$qfrom.$qjoin.$qwhere.$qgroup;

        $pwpdo->query($query,array('divid' => $division['id']));
        ...
        while ($row = $pwpdo->fetch_row())
        {
            ...
            $placeheld=array($division['id'],$row['party'],$ayevotes,$ayetells,$novotes,$notells,$boths,$abstentions,$possibles,$whip_guess);
...
function count_4d_info( $table, $group_by, $id, $votes_attended, $votes_possible) {
    ...
    $query = "
        ...
        LEFT JOIN pw_mp ON
            pw_division.house = pw_mp.house AND
            pw_division.division_date BETWEEN pw_mp.entered_house AND pw_mp.left_house
    ...
    if ($id == 'division_id') {
        $things = $pwpdo->fetch_all_rows('SELECT division_id AS id FROM pw_division', array());
    } elseif ($id == 'mp_id') {
        $things = $pwpdo->fetch_all_rows('SELECT mp_id AS id FROM pw_mp', array());
    }

    foreach ($things as $thing) {
        $pwpdo->query($query, array('id' => $thing['id']));
    }
...
function do_house_ranking($house) {
    ...
    if (count($rows) == 0) {
        print '['.date('Y-m-d H:i:s').'] calc_caches: do_house_ranking: No MPs found using curdate method for '.$house.PHP_EOL;
        $rows=$pwpdo->fetch_all_rows($mps_query_start .  "and left_house = '2011-03-23'",array($house));
        if (count($rows) == 0) {
            print '['.date('Y-m-d H:i:s').'] calc_caches: do_house_ranking: No MPs found using fixed date left house method for  '.$house.PHP_EOL;
